...
let items = [
  { url: queenbed, name: "Queen Bed", price: 130, category: "BedFrame" },
  { url: ikeadesk, name: "Ikea Desk", price: 49, category: "Desk" },
  { url: lshapeddesk, name: "L-Shaped Desk", price: 93, category: "Desk" }
];
    ...
    const filteredItems = items
      .filter(
        item =>
          ...
          item.category == this.state.activeKey
      )
      .map(item => {
        return (
          <div className="item">
      ...
      <React.Fragment>
        ...
        >
          {this.state.catogries.map((catogry, index) => (
            <Nav.Item>
              <Nav.Link eventKey={catogry}>{catogry}</Nav.Link>
            </Nav.Item>
          ))}
        </Nav>
